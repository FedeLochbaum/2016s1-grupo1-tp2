class AfterExecution < WhenExecution
  ...
  def execute_code a_class, a_selector, a_block
    case (a_class.instance_method a_selector).arity
      ...
      when 0  then Proc.new { returnment = self.send(a_selector); a_block.call; returnment}
      when 1  then Proc.new {|x| returnment = self.send(a_selector, x); a_block.call; returnment}
